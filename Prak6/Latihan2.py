...
def generate_subkeys(key_binary):
    key_binary = permute(key_binary, PC1)
    C0 = key_binary[:28]
    D0 = key_binary[28:]
    subkeys = []
    for i in range(16):
...
def feistel_round(L, R, subkey):
    ...
    xor_result = ''.join(str(int(a) ^ int(b)) for a, b in zip(expanded_R, subkey))
    s_box_output = s_box_substitution(xor_result)
    p_permuted = permute(s_box_output, P)
    new_R = ''.join(str(int(a) ^ int(b)) for a, b in zip(L, p_permuted))
    return R, new_R

def des_encrypt(plaintext_binary, key_binary):
    ...
    for i in range(0, len(plaintext_binary), 64):
        ...
        for round_num in range(16):
            L, R = feistel_round(L, R, subkeys[round_num])
        combined = R + L
        ciphertext += permute(combined, IP_INV)
    
    return subkeys, ciphertext
...
def main():
    ...
    if len(key) > 8:
        print("Kunci maksimal 8 karakter!")
        return
    
    # Pad key to 8 characters if necessary
    key = key.ljust(8, '\0')
    
    plaintext_binary = string_to_binary(plaintext)
    key_binary = string_to_binary(key)
    
    subkeys, ciphertext_binary = des_encrypt(plaintext_binary, key_binary)
    
    print("\n16 Subkey:")
    for i, subkey in enumerate(subkeys, 1):
        print(f"K{i}: {subkey}")
    
    print(f"\nCiphertext (Binary): {ciphertext_binary}")
    print(f"Ciphertext (Hex): {binary_to_hex(ciphertext_binary)}")
